...
class MedbotLoginGui:
    def __init__(self, medbot):
        ...
        placeholder = tkinter.Canvas(self.window, width = 380, height = 400)
        qrcode = ImageTk.PhotoImage(Image.open('images/qrcode.png').resize((128,166)))
        placeholder.create_text(190, 65, text = 'Med-bot: Pulse Rate\nand\nBlood Pressure Monitor',
                                anchor = tkinter.CENTER, font = ('Lucida',14), justify = 'center')
        placeholder.create_image(125, 130, image = qrcode, anchor = tkinter.NW)
        placeholder.create_text(180, 355, text = 'Place your QR Code\nwithin the frame to Login',
                                anchor = tkinter.CENTER, font = ('Lucida',14), justify = 'center')
